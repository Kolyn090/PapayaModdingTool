    ...
    public class TextureEncoderDecoder
    {
        ...
        private static byte[] DecodePVRTexLib(byte[] data, int width, int height, TextureFormat format)
        {
            ...
            unsafe
            {
                ...
                fixed (byte* destPtr = dest)
                {
                    ...
                    size = PVRTexLibWrapper.DecodeByPVRTexLib(dataIntPtr, destIntPtr, (int)format, (uint)width, (uint)height);
                }
            }
            if (size > 0)
            {
        ...
        public static byte[] EncodeMip(byte[] data, int width, int height, TextureFormat format, int quality, int mips = 1)
        {
            switch (format)
            {
                ...
                case TextureFormat.ASTC_RGBA_12x12:
                    {
                        byte[] res = PVRTexLibWrapper.EncodePVRTexLib(data, width, height, (UnityEngine.TextureFormat)format, quality);
                        return res;
                    }
                case TextureFormat.DXT1:
                case TextureFormat.DXT5:
                case TextureFormat.BC7:
                    {
                        byte[] res = ISPCWrapper.EncodeISPC(data, width, height, format, quality);
                        return res;
                    }
